...
import { Crown, MapPin, Calendar, Clock, User, ArrowRight, CheckCircle, LogOut } from "lucide-react";
import StatusBadge from "@/components/shared/StatusBadge";
import { motion, AnimatePresence } from "framer-motion";

export default function TenantDriverPortal() {
  ...
  const { data: bookings = [] } = useQuery({
    queryKey: ["tenant-driver-bookings", driver?.id, slug],
  ...
  const updateBooking = useMutation({
    ...
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ["tenant-driver-bookings"] }),
  });
    ...
    return (
      <div className="min-h-screen bg-background flex items-center justify-center p-4">
        <div className="text-center space-y-4">
          ...
          <p className="text-muted-foreground">{tenant?.business_name || "Service provider"}</p>
          <p className="text-sm text-muted-foreground">No driver profile found for your account.<br />Contact your dispatcher to set up access.</p>
        </div>
      </div>
    );
  }
      ...
      <header className="border-b border-border/50 bg-card/80 backdrop-blur-sm sticky top-0 z-10">
        <div className="max-w-lg mx-auto px-4 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 rounded-lg flex items-center justify-center" style={{ background: tenant?.primary_color || "#d4a017" }}>
          ...
          <div className="flex items-center gap-2">
            <Select value={driver.status || "offline"} onValueChange={v => updateDriverStatus.mutate({ status: v })}>
